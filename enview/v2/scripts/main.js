...
window.onload = function() {
   ...
   //      .addTo(map);
	...
	var inner = L.canvasOverlay()
        .drawing(drawingOnCanvas.bind(L, {alpha: 0.5, donutwidth: 15}))
        .addTo(map);
	var outer = L.canvasOverlay()
        .drawing(drawingOnCanvas.bind(L, {alpha: 1, donutwidth: 2}))
        .addTo(map);

    function drawingOnCanvas(options, canvasOverlay, params) {
		...
		*/
		...
		for(i=0; i<points.length; i++) {
		    ...
		    renderScannerFill.call(ctx, points[i], points[i].maxr, chroma(colors[i % 2]).alpha(options.alpha).css() );
		}
		ctx.save();	
		...
		ctx.globalCompositeOperation = "destination-out";
		for(i=0; i<points.length; i++) {
		    renderScannerFill.call(ctx, points[i], points[i].maxr-options.donutwidth , "rgba(0,0,0,1)");
		}
		ctx.save();	
